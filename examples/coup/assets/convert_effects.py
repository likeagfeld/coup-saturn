...
def quantize_sequence(frames, name, mask_fn=None):
    ...
    masks = []
    for im in frames:
        rgb = repair_edges(im.convert("RGB"))
...
def wordmark_rgb555_bytes(im, mask_fn):
    """RGB555 (colour mode 5) texel encoding of the wordmark, for the
    gouraud-lit sheen.
    ...
    the RGB mode ... If not in the RGB mode (characters in color bank mode),
    the results cannot be guaranteed." The wordmark's existing texture
    (coup_ui_wordmark_f00 below) is 4bpp colour-BANK data - exactly what that
    passage rules out - so the sheen needs its own RGB-mode copy rather than
    reusing it.

    Mirrors quantize_sequence()'s own mask_fn handling (mask_fn=None ->
    magenta-key + defringe; otherwise mask_fn(rgb)) so this produces the
    IDENTICAL transparent/opaque pixel set as the 4bpp version it stands in
    for - the two must agree on which pixels are letterforms.

    Per-pixel encoding (VDP1_Manual.txt:3873-3874, "the RGB code is
    represented by a MSB (value 1)"; :3747, mode-5 transparent code is
    "0000H"): opaque pixels get rgb_to_saturn555() | 0x8000, transparent
    pixels get exactly 0x0000. Packed big-endian (high byte first) per pixel,
    matching how a 16-bit store lands in memory on the big-endian SH-2 -
    saturn_vdp1_upload_texture() copies these bytes to VRAM unchanged, the
    same byte-for-byte path every other 4bpp texture in this file already
    uses, so no new upload primitive is needed.
    """
    rgb = im.convert("RGB")
    if mask_fn is None:
        rgb = defringe(rgb)
        mask = [not is_key(p) for p in rgb.getdata()]
    else:
        mask = mask_fn(rgb)
    ...
    for y in range(h):
        for x in range(w):
            if mask[i]:
                r, g, b = px[x, y][:3]
                word = rgb_to_saturn555(r, g, b) | 0x8000
            else:
                word = 0x0000
...
def emit(sequences, singles, out_dir, extra_raw16=None):
    ...
    with open(path, "w") as f:
        ...
        if extra_raw16:
            f.write("/* Raw RGB555 (colour mode 5) texel data - NOT palette "
                    "indices. See convert_effects.py:wordmark_rgb555_bytes "
                    "for why the wordmark needs a second, non-4bpp copy for "
                    "the gouraud sheen (ST-013-R3 p.94). Named "
                    "coup_ui_<name>_f00 to match the coup_(fx|ui)_NAME_fNN "
                    "pattern every other asset uses, so "
                    "scripts/qa/verify_facelift.py gate D counts these bytes "
                    "in its VDP1 VRAM budget automatically. */\n")
            for name, blob, w, h in extra_raw16:
                f.write(f"#define COUP_UI_{name.upper()}_W {w}\n")
                f.write(f"#define COUP_UI_{name.upper()}_H {h}\n")
                f.write(f"static const uint8_t coup_ui_{name}_f00"
                        f"[{len(blob)}] = {{\n")
                for i in range(0, len(blob), 16):
                    f.write("    " + ", ".join(f"0x{b:02X}"
                                               for b in blob[i:i + 16]) + ",\n")
...
def main():
    ...
    ap.add_argument("--wordmark-rgb555", action="store_true",
                    help="ALSO emit an RGB555 (colour mode 5) copy of the "
                    "wordmark for gouraud lighting (saturn_vdp1_draw_"
                    "sprite_gouraud). Off by default - MEASURED 2026-08-06: "
                    "this costs 32,768 B, and this bare-SGL build's whole "
                    "program (.text+.rodata, not just .data/.bss) loads "
                    "into WRAM-H at boot, so this is a WRAM-H cost, not "
                    "just a VDP1 VRAM one. Against a 30,188 B baseline "
                    "slack under SGL's SortList (scripts/qa/"
                    "verify_facelift.py gate F), the texture alone does not "
                    "fit; do not pass this flag until the sprite is "
                    "streamed from CD like the background scenes are, "
                    "rather than linked into the resident binary.")
    args = ap.parse_args()
    ...
    if os.path.exists(logo):
        ...
        # Second, RGB555 copy for the gouraud sheen - see
        # wordmark_rgb555_bytes() for why the 4bpp copy above cannot be
        # gouraud-shaded (ST-013-R3 p.94). Opt-in only - see --wordmark-
        # rgb555's help text for the MEASURED WRAM-H budget finding that
        # keeps this off by default.
        if args.wordmark_rgb555:
            rgb_blob = wordmark_rgb555_bytes(im, logo_mask)
            extra_raw16.append(("wordmarkrgb", rgb_blob, w, h))
            total += len(rgb_blob)
            print(f"  ui {'wordmark(rgb)':12}  1 frame  {w:3}x{h:<3} RGB555 "
                  f" {len(rgb_blob):>6,} B  (gouraud sheen texture, "
                  f"OPT-IN - see --wordmark-rgb555 help)")
    ...
    path = emit(sequences, singles, args.out, extra_raw16=extra_raw16)
    print(f"-> {path}")
